...
type NodeViewMode = "list" | "map";
type UrlTestResult = { node_id: string; latency_ms: number; active_tunnel: boolean };
type GeoPoint = { host: string; ip: string; latitude: number; longitude: number; country: string; country_code: string };
...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<NodeViewMode>("list");
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (groupMenuOpen) setGroupMenuOpen(false); else if (page === "settings") setPage("main");
    };
  ...
  async function chooseNode(groupId: string, nodeId: string) {
    ...
    try {
      ...
      } else setSelection(await invoke<SelectionView>("select_node", { groupId, nodeId }));
    } catch (reason) {
  ...
  async function testNode(group: GroupView, node: NodeView) {
    if (testingNodeId || vpn.phase === "starting" || vpn.phase === "stopping") return;
    setTestingNodeId(node.id); setError("");
    try {
    ...
    finally { setTestingNodeId(null); }
  }

  async function refresh(group = visibleGroup) {
    if (!group) return;
    setBusy(true); setError("");
    try { await invoke("refresh_subscription", { groupId: group.id }); setLatencies({}); await reloadData(); }
  ...
  return <main className="shell">
    {page === "main" ? <div className="main-page">
      <header className="topbar"><h1>dot.</h1><div className="topbar-actions"><small>v0.2.0-alpha.1</small><button className="icon-button" onClick={() => setPage("settings")} aria-label="settings">⚙</button></div></header>
      <div className="dashboard-layout">
        <section className="connection-pane">
          ...
          {activeGroup && activeNode && <button className="connection-test" disabled={!!testingNodeId || vpn.phase === "starting" || vpn.phase === "stopping"} onClick={() => testNode(activeGroup, activeNode)}>
            <span>URL TEST</span><strong>{testingNodeId === activeNode.id ? "TESTING…" : latencies[activeNode.id] ? `${latencies[activeNode.id]} ms` : "READY"}</strong>
          </button>}
        ...
        <section className="nodes-pane">
          <NodeToolbar groups={groups} group={visibleGroup} menuOpen={groupMenuOpen} busy={busy} viewMode={viewMode} onViewMode={setViewMode} onToggleMenu={() => setGroupMenuOpen(value => !value)} onChooseGroup={groupId => { setViewGroupId(groupId); setGroupMenuOpen(false); }} onRefresh={() => refresh(visibleGroup)} onOpenSettings={() => { setGroupMenuOpen(false); setPage("settings"); }} />
          {!visibleGroup ? <EmptyNodes title="no subscriptions" detail="add a subscription in settings." action="OPEN SETTINGS" onAction={() => setPage("settings")} /> : visibleGroup.nodes.length === 0 ? <EmptyNodes title={`no nodes in ${visibleGroup.name}`} detail="refresh this subscription or check its URL." action="REFRESH" onAction={() => refresh(visibleGroup)} /> : viewMode === "map" ?
            <NodeMap group={visibleGroup} activeNode={activeNode} latencies={latencies} testingNodeId={testingNodeId} busy={busy} onChoose={chooseNode} onTest={testNode} onError={setError} /> :
            <div className="node-list" role="list">{visibleGroup.nodes.map(node => <NodeRow key={node.id} node={node} selected={visibleGroup.id === activeGroup?.id && node.id === activeNode?.id} live={vpn.phase === "connected" && visibleGroup.id === activeGroup?.id && node.id === activeNode?.id} switching={busy && visibleGroup.id === activeGroup?.id && node.id === activeNode?.id && vpn.phase === "stopping"} disabled={busy} latency={latencies[node.id]} testing={testingNodeId === node.id} onClick={() => chooseNode(visibleGroup.id, node.id)} onTest={() => testNode(visibleGroup, node)} />)}</div>}
        </section>
      </div>
...
function NodeToolbar({ groups, group, menuOpen, busy, viewMode, onViewMode, onToggleMenu, onChooseGroup, onRefresh, onOpenSettings }: { groups: GroupView[]; group?: GroupView; menuOpen: boolean; busy: boolean; viewMode: NodeViewMode; onViewMode: (mode: NodeViewMode) => void; onToggleMenu: () => void; onChooseGroup: (id: string) => void; onRefresh: () => void; onOpenSettings: () => void }) {
  return <><div className="node-toolbar"><div className="group-picker"><button className="group-button" onClick={onToggleMenu} aria-expanded={menuOpen}><strong>{group?.name ?? "subscriptions"}</strong><span>⌄</span></button>{menuOpen && <div className="group-menu"><div className="group-menu-title">subscriptions.</div>{groups.map(item => <button key={item.id} className={item.id === group?.id ? "group-menu-item selected" : "group-menu-item"} onClick={() => onChooseGroup(item.id)}><span>{item.name}</span><small>{item.nodes.length}</small></button>)}<button className="group-menu-item add" onClick={onOpenSettings}>+ add / manage</button></div>}</div><div className="node-tools"><small>{group?.nodes.length ?? 0} nodes</small><button className="refresh-button" disabled={!group || busy} onClick={onRefresh} aria-label="refresh subscription">↻</button></div></div>{group && group.nodes.length > 0 && <div className="view-switch"><button className={viewMode === "list" ? "active" : ""} onClick={() => onViewMode("list")}>LIST</button><button className={viewMode === "map" ? "active" : ""} onClick={() => onViewMode("map")}>MAP</button></div>}</>;
}

function NodeRow({ node, selected, live, switching, disabled, latency, testing, onClick, onTest }: { node: NodeView; selected: boolean; live: boolean; switching: boolean; disabled: boolean; latency?: number; testing: boolean; onClick: () => void; onTest: () => void }) {
  return <div className={`node-row ${selected ? "selected" : ""} ${live ? "live" : ""}`} role="listitem"><button className="node-main" disabled={disabled} onClick={onClick}><span className="node-marker" /><span className="node-copy"><span className="node-name">{node.name}</span><small>{node.security} · {node.transport} · {node.host}:{node.port}</small></span><span className="node-state">{switching ? <em>SWITCHING</em> : live ? <em>LIVE</em> : selected ? <em>SELECTED</em> : <span>›</span>}</span></button><button className="node-test" disabled={disabled || testing} onClick={onTest}>{testing ? "··" : latency ? `${latency} ms` : "TEST"}</button></div>;
}

function NodeMap({ group, activeNode, latencies, testingNodeId, busy, onChoose, onTest, onError }: { group: GroupView; activeNode?: NodeView; latencies: Record<string, number>; testingNodeId: string | null; busy: boolean; onChoose: (groupId: string, nodeId: string) => void; onTest: (group: GroupView, node: NodeView) => void; onError: (message: string) => void }) {
  const container = useRef<HTMLDivElement | null>(null); const mapRef = useRef<maplibregl.Map | null>(null); const markers = useRef<maplibregl.Marker[]>([]); const [points, setPoints] = useState<Record<string, GeoPoint>>({}); const [loading, setLoading] = useState(false);
  useEffect(() => { let cancelled = false; setPoints({}); setLoading(true); Promise.allSettled(group.nodes.map(async node => ({ node, geo: await invoke<GeoPoint>("node_geo", { host: node.host }) }))).then(results => { if (cancelled) return; const next: Record<string, GeoPoint> = {}; results.forEach(result => { if (result.status === "fulfilled") next[result.value.node.id] = result.value.geo; }); setPoints(next); setLoading(false); }).catch(reason => { if (!cancelled) { setLoading(false); onError(String(reason)); } }); return () => { cancelled = true; }; }, [group.id]);
  useEffect(() => { if (!container.current || mapRef.current) return; const map = new maplibregl.Map({ container: container.current, center: [15, 50], zoom: 3.1, minZoom: 1.5, maxZoom: 12, attributionControl: false, style: { version: 8, sources: { osm: { type: "raster", tiles: ["https://tile.openstreetmap.org/{z}/{x}/{y}.png"], tileSize: 256 } }, layers: [{ id: "osm", type: "raster", source: "osm", paint: { "raster-saturation": -1, "raster-brightness-max": 0.42, "raster-contrast": 0.2 } }] } }); map.addControl(new maplibregl.NavigationControl({ showCompass: false }), "bottom-right"); mapRef.current = map; return () => { map.remove(); mapRef.current = null; }; }, []);
  useEffect(() => { const map = mapRef.current; if (!map) return; markers.current.forEach(marker => marker.remove()); markers.current = []; const byCountry: Record<string, NodeView[]> = {}; group.nodes.forEach(node => { const geo = points[node.id]; if (!geo) return; (byCountry[geo.country_code || node.id] ||= []).push(node); }); Object.values(byCountry).forEach(nodes => nodes.forEach((node, index) => { const geo = points[node.id]; const angle = index * 2.399; const radius = index === 0 ? 0 : 0.18 + Math.floor(index / 6) * 0.12; const el = document.createElement("button"); el.className = `map-node ${activeNode?.id === node.id ? "active" : ""}`; el.title = `${node.name}${latencies[node.id] ? ` · ${latencies[node.id]} ms` : ""}`; el.onclick = () => onChoose(group.id, node.id); const marker = new maplibregl.Marker({ element: el, anchor: "center" }).setLngLat([geo.longitude + Math.cos(angle) * radius, geo.latitude + Math.sin(angle) * radius]).addTo(map); markers.current.push(marker); })); }, [points, activeNode?.id, latencies, busy]);
  return <div className="map-wrap"><div ref={container} className="node-map" />{loading && <div className="map-loading">LOCATING NODES…</div>}<div className="map-footer"><span>{Object.keys(points).length}/{group.nodes.length} LOCATED</span>{activeNode && <button disabled={busy || testingNodeId === activeNode.id} onClick={() => onTest(group, activeNode)}>{testingNodeId === activeNode.id ? "TESTING…" : latencies[activeNode.id] ? `${latencies[activeNode.id]} ms` : "TEST SELECTED"}</button>}</div></div>;
}
...
function TrafficCell({ label, arrow, value, total, align = "left" }: { label: string; arrow: string; value: string; total: string; align?: "left" | "right" }) { return <div className={`traffic-cell ${align === "right" ? "align-right" : ""}`}><small>{label}</small><strong>{arrow} {value}</strong><em>{total}</em></div>; }
...
function Toggle({ label, detail, checked, onChange }: { label: string; detail: string; checked: boolean; onChange: (value: boolean) => void }) { return <button className="toggle" onClick={() => onChange(!checked)}><div><strong>{label}</strong><small>{detail}</small></div><span className={checked ? "switch on" : "switch"}><i /></span></button>; }
function connectionLabel(phase: EngineSnapshot["phase"]) { switch (phase) { case "starting": return "connecting"; case "stopping": return "disconnecting"; default: return phase; } }
...
function formatTime(value: number) { const hours = Math.floor(value / 3600), minutes = Math.floor((value % 3600) / 60), seconds = value % 60; return [hours, minutes, seconds].map(part => String(part).padStart(2, "0")).join(":"); }
