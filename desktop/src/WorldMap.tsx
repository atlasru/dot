...
export default function WorldMap({ markers, selectedCountry, onMarkerClick }: {
  ...
  useEffect(() => {
    let cancelled = false;
    loadGeometry().then(value => { if (!cancelled) setCountries(value); }).catch(() => { if (!cancelled) setGeometryError(true); });
  ...
  return <div ref={wrapRef} className="world-map-wrap">
    {geometryError && <span className="map-error" role="status">Map background unavailable. Located nodes remain selectable.</span>}
    <canvas
      ref={canvasRef}
